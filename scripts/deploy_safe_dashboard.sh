...
# Em modo inline a api faz fork() dos bot-workers; `pm2 stop api` para só o
# processo pai e os forks viram órfãos segurando conexões WAL no .db, o que
# trava o DDL com `database is locked` (vide AGENTS.md pegadinha #8). Resolvemos
# o caminho do banco a partir do DATABASE_URL do .env DESTE ambiente e usamos
# apenas os arquivos dentro de $ROOT_DIR/prisma — sem risco de cruzar ambientes.
kill_db_holders_for_migration() {
  ...
  if command -v fuser >/dev/null 2>&1; then
    if fuser -s "$db_path" "$db_path-wal" "$db_path-shm" 2>/dev/null; then
      ...
      sleep 2
      # SIGKILL nos teimosos que não saíram com TERM.
