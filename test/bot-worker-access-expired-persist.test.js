...
// consumidor em sessionCore.js) e saía com process.exit(0). WaSession.status
...
test('bloco de acesso expirado persiste WaSession antes de sair do processo', () => {
  ...
  const blockEnd = botWorkerSource.indexOf('process.exit(0)', blockStart)
  assert.notEqual(blockEnd, -1, 'process.exit(0) não encontrado dentro do bloco')

  const block = botWorkerSource.slice(blockStart, blockEnd)
  assert.match(
    block,
    /persistSessionPatch\(\s*\{\s*status:\s*'disconnected'/,
    'accessExpiresAt vencido precisa persistir status=disconnected no WaSession antes de process.exit(0), senão o health monitor do supervisor tenta ressuscitar a sessão pra sempre',
  )
})
