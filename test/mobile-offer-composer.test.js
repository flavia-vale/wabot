...
test('{cupom} sobrevive intacto a applyTemplateVariables QUANDO pedido (resolvido só no envio)', () => {
  const result = applyTemplateVariables('Oferta: {produto}\n{cupom}\n{link}', {
    title: 'Produto X',
    link: 'https://loja.test/produto',
    keepCouponToken: true,
  })
  assert.match(result, /\{cupom\}/, 'com keepCouponToken o marcador fica para o robô trocar no envio')
  // Sem pedir, o padrão é apagar: tela que publica ou copia sem passar pelo
  // robô não pode deixar o marcador cru chegar ao grupo (RCA 19/09).
  const semPedir = applyTemplateVariables('Oferta: {produto}\n{cupom}\n{link}', { title: 'Produto X', link: 'https://loja.test/produto' })
  assert.doesNotMatch(semPedir, /\{cupom\}/)
})
