...
- [X] T022 [P1] Invalidar o cache de sondagem Amazon (`amazonSessionProbeCache.js`) quando a credencial `amazon` é regravada em `PUT /:platform` (`src/api/routes/credentials.js`). Hoje, após a usuária recadastrar um cookie novo (fluxo principal da US1/SC-004 quando a sessão expira), o `GET /amazon/session` continua servindo o resultado antigo em cache (ex.: `alive:false`/expired) por até `AMAZON_SESSION_PROBE_CACHE_TTL_MS` (default 5min) — o painel segue mostrando "expirado" mesmo com cookie válido recém-salvo. Corrigir: exportar `invalidateCachedProbe(userId)` (delete no `Map`) em `amazonSessionProbeCache.js`, injetá-lo em `credentialsRoutes` (mesmo padrão de opts das demais deps) e chamá-lo no handler do PUT quando `platform === 'amazon'` (após o upsert). Cobrir com teste de rota: PUT amazon → GET amazon/session sonda de novo (cache miss), não serve o valor stale. (review)
- [X] T023 [P2] Não cachear resultados indeterminados/transitórios na rota `GET /amazon/session` (`src/api/routes/credentials.js`). Hoje `setCachedProbe` grava QUALQUER `publicResult`, inclusive `alive:null, reason:'network_error'` — um único blip transitório da Amazon fixa o painel em "network_error" pela janela inteira do TTL e impede refletir a sessão realmente viva por até 5min. Corrigir: só popular o cache quando o resultado é definitivo (`alive === true` ou `alive === false`); estados indeterminados (`alive === null`) devem re-sondar na próxima abertura. Cobrir com teste: probe transitório não é servido do cache na chamada seguinte. (review)
- [X] T024 [P3] Higiene de memória do `amazonSessionProbeCache.js`: `pruneExpired` é exportado mas nunca é chamado em `src/` — entradas de usuários que sondam uma única vez e não retornam permanecem no `Map` até um novo acesso do mesmo `userId` (eviction só é lazy no `getCachedProbe`). Footprint é pequeno (bounded por nº de usuários distintos), mas para não deixar código morto: ou agendar `pruneExpired` num timer `unref()` de baixa frequência (padrão de `startLoginAttemptsCleanup`), ou remover a export se a poda lazy for considerada suficiente e documentar a decisão. (review)
