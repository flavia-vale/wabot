...
| 7 | **Scrape de item via API OAuth** | `src/converters/productInfoScraper.js:353-379` (`getMlUserToken`) | OAuth `oauthRefreshToken` | **Sim** (refresh → novo access + novo refresh token) | **NÃO — descarta o token renovado** ⚠️ |
| 8 | Motor de oferta / template espelhado | `src/converters/offerEngine.js:151,205`; `src/core/mirrorTemplate.js:89` | OAuth (passa `mlCredentials` puro) | via #7 | via #7 (herda o gap) |
...
### C2 — **Refresh OAuth descarta o token rotacionado → CONFIRMADA (causa raiz primária do eixo OAuth)**
Evidência: `getMlUserToken` (`productInfoScraper.js:353-379`) detecta access token expirado (`Date.now() >= oauthTokenExpiry`), faz `grant_type=refresh_token` e retorna `data.access_token` — **sem persistir** `oauthAccessToken`, o novo `oauthTokenExpiry`, nem o `refresh_token` que o ML devolve na resposta. O refresh token do ML é **rotativo/single-use**: cada refresh invalida o refresh token anterior e emite um novo. Como o novo não é gravado, o próximo refresh reenvia um refresh token já invalidado → o ML recusa → a sessão OAuth "morre" e a cliente precisa refazer o fluxo OAuth. Não há `__onCredentialPatch` nesse caminho (o `mlCredentials` chega como objeto puro de `offerEngine`/`mirrorTemplate`). É o análogo funcional exato do bug de "reenviar token velho" da Amazon, no eixo OAuth.

### C3 — Sondagem do painel sem cache batendo no ML a cada load → **CONFIRMADA (agravante)**
Evidência: a rota `GET /mercadolivre/session` (`credentials.js:37-53`) **não** tem cache TTL — compare com `GET /amazon/session` (`credentials.js:67-101`), que ganhou `getCachedProbe`/`setCachedProbe`/`invalidateCachedProbe` na feature 001. Cada carregamento do painel dispara um `createLink`-probe real (via `checkMercadoLivreSession`). N aberturas = N chamadas ao ML, aumentando tráfego, consumo de rotação de cookie e risco de padrão anômalo. Corrige-se com cache curto por usuário (FR-005/SC-003), reusando `amazonSessionProbeCache.js`.
...
| Cache TTL curto por usuário na sondagem | **Aplica — a fazer** | `GET /mercadolivre/session` ainda não tem; reusar `amazonSessionProbeCache.js` |
| Retornar patch em vez de persistir via gancho ausente | **Aplica ao OAuth** | estender o contrato "retorna patch, chamador persiste" ao refresh OAuth |
