...
- [X] T030 [US4] Em `dashboard/lib/mobileOfferComposer.js`: remover `{linhaDeCupom}` de `OFFER_TEMPLATE_VARIABLE_GROUPS` (FR-019); em `applyTemplateVariables`, remover o parâmetro `couponLine` da assinatura, remover a linha do token **sempre** (não apenas quando vazio, como hoje) e substituir `{linhaDeCupom}` por `''` incondicionalmente.
- [X] T031 [P] [US4] Confirmar que `UNRESOLVED_OFFER_PLACEHOLDER_RE` em `src/core/mirrorTemplate.js` continua varrendo `linhaDeCupom` como placeholder não resolvido (cinto e suspensório para quem digitar `{linhaDeCupom}` à mão num template novo, cenário 4 do US4) — ajustar a regex só se algo em T017/T030 tiver quebrado essa cobertura, sem remover a entrada da lista.
...
- [X] T037 Atualizar a documentação da feature para refletir o desenho vigente do marcador `{cupom}` na montagem: por padrão `applyTemplateVariables`/`buildMobileOfferText` (`dashboard/lib/mobileOfferComposer.js`) APAGAM o marcador; só os caminhos que passam pelo robô pedem `keepCouponToken: true` (`src/core/mirrorTemplate.js`, `src/offerAutomation/dispatcher.js`, `dashboard/app/painel/criar-oferta/page.js`, `dashboard/lib/offerTemplatePreview.js`) e o robô troca/apaga o marcador no envio; o "Copiar oferta" do Criar oferta usa `stripCouponToken`. Corrigir a frase "o token de cupom sobrevive à montagem do texto" em `research.md` (D3) e as passagens equivalentes em `plan.md` (decisão 1, árvore de arquivos, tabela dos três caminhos) e `data-model.md`, citando o guarda `test/criar-oferta-sem-preco.test.js` (RCA 19/09) e `test/cupom-marcador-montagem.test.js` como motivo — sem reverter código per plan: decisão D3 (partial)
- [X] T038 Reescrever o FR-025 de `spec.md` (e a linha "Fora de escopo" de Assumptions, mais a menção em `research.md` D3(b)) para dizer o que de fato vale: o Criar oferta não ganha seletor nem pré-visualização de cupom, mas o texto que ele monta preserva `{cupom}` e, como todo envio do painel passa pelo robô, "Enviar agora" e "Inserir na fila" publicam com o melhor cupom da loja (preço desconhecido → ordem fixa do FR-011), "Agendar" publica sem cupom (o ramo `scheduled` não leva `couponContext`, o marcador é só apagado) e "Copiar oferta" sai sem marcador — registrar isso como decisão explícita da dona do produto para a fila per FR-025 (contradicts)
- [X] T039 Atualizar a "Nota de operação" de `spec.md`, a linha "Nota do modo `remote`" e o risco correspondente em `plan.md`, a consequência operacional em `research.md` (D1) e a §4 de `quickstart.md`: desde 2026-08-26 `deploy_safe_dashboard.sh`/`deploy_safe_staging.sh` reiniciam o `bot-supervisor` sozinhos quando o diff toca `WORKER_CODE_PATHS_RE` — o que esta feature faz (`src/bot-worker.js`, `src/core/`, `prisma/schema.prisma`) —, então o deploy JÁ reconecta todas as sessões e precisa ser anunciado antes; o `pm2 restart bot-supervisor --update-env` manual fica só para `RESTART_SUPERVISOR=0` ou deploy fora do script per spec: Nota de operação (partial)
