...
- [ ] T001 Rodar `npm ci` na raiz do repo e `npm ci` dentro de `dashboard/` para garantir
      dependências instaladas (nenhuma dependência nova é introduzida por esta feature).
- [ ] T002 Capturar a baseline vermelha rodando, de dentro de `dashboard/`: `npm run
...
- [ ] T003 [P] [US1] Adicionar a entrada `/cadastro` em `CONTENT_SEO_ROUTES` em
...
- [ ] T004 [P] [US1] Adicionar a entrada `/parcerias` em `CONTENT_SEO_ROUTES` em
      ...
      página).
- [ ] T005 [US1] Em `dashboard/lib/seo-registry.mjs`, diferenciar a entrada existente de
...
- [ ] T006 [US1] Rodar `npm run guard:seo-registry` de dentro de `dashboard/` e confirmar exit
      0, sem `/cadastro` nem `/parcerias` na lista de rotas ausentes (verifica T003/T004).
- [ ] T007 [US1] Rodar `npm run lint:seo-metadata` de dentro de `dashboard/` e confirmar exit
      0, sem o par duplicado `/conteudos` × `/blog/comecar-afiliado-whatsapp-sem-grupo-grande`
      (verifica T005).
- [ ] T008 [US1] Rodar `npm run validate:seo-p0` de dentro de `dashboard/` e confirmar que a
...
- [ ] T009 [P] [US2] Em `dashboard/lib/marketing-attribution.js`, adicionar uma função de
...
- [ ] T010 [P] [US2] Em `dashboard/components/marketing/OrganicPageTracker.jsx`, chamar a
...
- [ ] T011 [US2] Em `dashboard/app/login/page.js`, no fluxo de registro (`handleSubmit` /
...
- [ ] T012 [US2] Em `src/api/routes/auth.js` (`POST /register`), aceitar `landingPage`
...
- [ ] T013 [P] [US2] Adicionar rota `GET /marketing/signups-by-landing` em
...
- [ ] T014 [P] [US2] Adicionar o client wrapper `adminMarketingSignupsByLanding(params)` em
      `dashboard/lib/api.js` (mesmo padrão de `adminMarketingCampaigns`/`adminMarketingFunnel`),
      consumindo a rota de T013.
- [ ] T015 [US2] Em `dashboard/app/admin/marketing-growth/page.js`, adicionar um card/tabela
...
- [ ] T016 [P] [US2] Adicionar teste `test/register-landing-attribution.test.js` cobrindo a
...
- [ ] T018 [P] [US3] Verificar em `dashboard/components/marketing/_preservationBlogPosts.js`
...
- [ ] T019 [US3] Em `dashboard/components/marketing/LeadMagnetCard.jsx`, ajustar a validação
...
- [ ] T020 [US3] Revisar toda a cópia visível de `dashboard/components/marketing/LeadMagnetCard.jsx`
...
- [ ] T022 [US4] Gerar/inspecionar o sitemap (`dashboard/app/sitemap.js`, que deriva 100% de
...
- [ ] T023 [P] [US4] Inspecionar `dashboard/app/robots.js` e confirmar que a lista `allow`/
...
- [ ] T024 [P] [US4] Criar `docs/deploy/seo-search-console-submission.md` documentando o passo
...
- [ ] T025 Confirmar o envelope de custo (SC-006/FR-013): rodar `grep "name:"
...
- [ ] T027 [P] Atualizar `specs/010-seo-lead-capture/quickstart.md` (ou anexar nota) com a
