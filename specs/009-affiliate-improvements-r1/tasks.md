...
- [X] T021 [US2] Atualizar `dashboard/app/painel/afiliados/page.js` com botão "Solicitar saque" (habilitado/desabilitado conforme `available.canRequest`) e histórico de solicitações.
- [X] T022 [US2] Atualizar a página admin de afiliados com a fila de solicitações de saque (`GET /admin/affiliates/payout-requests`) e ações de confirmar/recusar.
...
- [X] T023 [P] [US3] Criar módulo puro `src/domain/affiliate/pixKeyValidation.js`: `validatePixKey({ pixKey, pixKeyType })` → `{ ok, error }` com regras por tipo (`cpf`: 11 dígitos + dígito verificador; `phone`: E.164 BR celular `+55DDD9XXXXXXXX`; `email`: regex de e-mail; `random`: UUID v4 ou EVP 32 hex), normalizando espaços/`.`/`-`/`(`/`)` antes de validar CPF/telefone, conforme `contracts/pix-validation.md`.
- [X] T024 [P] [US3] Criar `test/affiliate-pix-validation.test.js` cobrindo casos válidos e inválidos para os 4 tipos (incluindo CPF com dígito verificador errado e telefone sem DDD).
- [X] T025 [US3] Chamar `validatePixKey` em `applyAffiliate` (`src/domain/affiliate/service.js`, linha ~138) e em `PUT /affiliate/me` (`src/api/routes/affiliate.js`), retornando 400 com a mensagem de `validatePixKey` **antes** de qualquer chamada a `encryptCredential` (FR-021 preservado).
...
- [X] T026 [P] [US4] Criar módulo puro `src/domain/affiliate/orphanTouchPolicy.js`: `resolveOrphanTouchDecision({ orphanTouchWindowDays, orphanTouchMode, touchAgeDays })` → `{ withinWindow, shouldHold }` implementando a semântica de `orphanTouchMode` (`off`|`window`|`hold`|`both`) descrita em `contracts/affiliate-settings.md`.
- [X] T027 [P] [US4] Criar `test/affiliate-orphan-touch-policy.test.js` cobrindo os 4 modos × dentro/fora da janela.
- [X] T028 [US4] Estender `attachOrphanTouchesByDevice` em `src/domain/affiliate/service.js` (linha ~119, hoje `windowDays = 30` hardcoded) para ler `orphanTouchWindowDays`/`orphanTouchMode` de `getAffiliateSettings()`, usar `orphanTouchPolicy.resolveOrphanTouchDecision`, e criar a comissão resultante com `status='held'`/`heldAt`/`holdReason='orphan_device_attribution'` quando `shouldHold=true` (campos já existentes, sem migration nova). Nota de implementação: como `attachOrphanTouchesByDevice` roda no cadastro (antes de qualquer comissão existir), o hold é persistido sem migration nova via marcador `ORPHAN_DEVICE_HOLD_MARKER` no campo já existente `AffiliateAttributionTouch.campaign`; `tryCreateAffiliateCommission` lê esse marcador (quando presente, força `effectiveRisk.decision='hold'`/`reason='orphan_device_attribution'`) no momento em que a comissão é de fato criada (pagamento).
- [X] T029 [US4] Estender `test/affiliate-hardening.test.js` (onde já vivem os testes de `tryCreateAffiliateCommission`/R5/O1/O4) com casos: toque dentro da janela endurecida com modo `both`/`hold` → comissão `held`; toque fora da janela → nenhuma atribuição por dispositivo; modo `window` → comissão segue fluxo normal (sem hold); modo `off` → comportamento legado (30 dias, sem hold); e caso ponta-a-ponta confirmando que `tryCreateAffiliateCommission` nasce `held`/`holdReason='orphan_device_attribution'` quando o marcador está presente.
...
- [X] T034 [P] [US5] Criar `src/email/affiliateEmails.js` espelhando `src/email/welcomeEmail.js`: 4 pares `build*Email`/`send*Email` (aprovação, rejeição, comissão elegível, comissão paga), reaproveitando `sendMail`/`isEmailConfigured` de `src/email/mailer.js`, com skip silencioso para endereços de fallback (`user_*@sistema.com`, FR-024) e fire-and-forget (nunca lança para o chamador, FR-023).
- [X] T035 [P] [US5] Criar `test/affiliate-emails.test.js` (padrão de `test/welcome-email.test.js`) cobrindo: builders geram assunto/corpo esperado para os 4 eventos; sem `SMTP_*` configurado, `send*` retorna `{ skipped: true }`; endereço de fallback é ignorado mesmo com SMTP configurado.
- [X] T036 [US5] Disparar `sendAffiliateApprovedEmail`/`sendAffiliateRejectedEmail` fire-and-forget nas rotas admin de aprovação/rejeição de candidatura em `src/api/routes/affiliate.js`.
- [X] T037 [US5] Disparar `sendCommissionEligibleEmail` fire-and-forget em `promoteEligibleAffiliateCommissions` (`src/domain/affiliate/service.js`, linha ~545) quando uma comissão é promovida de `pending` para `eligible`.
- [X] T038 [US5] Disparar `sendCommissionPaidEmail` fire-and-forget nos pontos de marcação de pagamento existentes (mark-paid/mark-all-paid em `src/api/routes/affiliate.js`) e em `confirmPayoutRequest` (T016, US2) quando comissões viram `paid`.
...
- [X] T039 [P] [US6] Criar módulo puro `src/domain/affiliate/stuckPromotionAlarm.js`: `evaluateStuckPromotion({ rows, now, thresholdMs })` → `{ shouldAlarm, count, oldestMs }`, sem acesso a DB/env.
- [X] T040 [P] [US6] Criar `test/affiliate-stuck-promotion.test.js` cobrindo: nenhuma comissão vencida além do limiar → `shouldAlarm:false`; comissões vencidas além do limiar → `shouldAlarm:true` com `count`/`oldestMs` corretos; e `checkStuckPromotions` (fake db) chamando/não chamando `log.error`.
- [X] T041 [US6] Adicionar `'ops_affiliate_promotion_stuck'` à allowlist de `src/analytics.js` (mesmo padrão dos demais `ops_*`, linhas ~57-89).
- [X] T042 [US6] Implementar `checkStuckPromotions({ db, log, now, thresholdMs })` em `src/domain/affiliate/service.js` (consulta comissões `pending` com `eligibleAt < now - thresholdMs`, usa `stuckPromotionAlarm.evaluateStuckPromotion`, emite `log.error` + `AnalyticsEvent('ops_affiliate_promotion_stuck')` via `trackAnalyticsEventSafe` — dynamic import lazy best-effort, mesmo padrão de `src/observability/operationalSignals.js` — quando `shouldAlarm`) e chamá-la em `src/api/routes/payments.js` logo após `promoteEligibleAffiliateCommissions` no `reconciliationTimer`, lendo o limiar de `AFFILIATE_STUCK_PROMOTION_THRESHOLD_MS` (default 24h).
...
- [X] T043 Rodar a suíte completa `node --test` e confirmar que todos os testes novos e existentes passam (regressão zero), incluindo os arquivos listados em quickstart.md. `npm test` (1665 testes, 1661 passaram, 4 skipped pré-existentes, 0 falhas) — corrigido de quebra um teste stale em `test/affiliate-commission-reconciliation.test.js` (fake db desatualizado em relação ao `reverseAffiliateCommissionForPayment` já commitado em US1: passou a capturar `findMany` antes do `updateMany` e a atualizar por `id` de comissão, não mais por `paymentId` em bulk).
- [X] T044 Seguir o roteiro de `quickstart.md` (US1–US7) e a verificação final SC-009 (`AffiliateAttributionTouch.ipHash/uaHash` não recebem novos valores por esta feature) antes de abrir PR para `develop`. Parte local concluída: suíte db-free/env-free listada em quickstart.md (45 testes) verde; `git diff` de `service.js` confirma que `attachOrphanTouchesByDevice` só LÊ `ipHash`/`uaHash` para matching (comportamento preexistente) — nenhum novo valor é gravado (FR-031). Os passos que exigem staging real (webhook MP de estorno, `curl` autenticado, e-mail com SMTP configurado) ficam pendentes de execução manual pós-merge — ver T045.
- [X] T045 ⏸️ DIFERIDA (validação manual pós-merge, NÃO executada nesta sessão) — Validar em staging (após merge em `develop` e autodeploy): aplicar migration (`npx prisma migrate deploy`), conferir `PRAGMA table_info('AffiliateSettings')` e `.tables` para `AffiliatePayoutRequest`, e repetir os testes de rota/painel do quickstart.md antes de abrir PR `develop → main`. **BLOQUEADA nesta sessão**: exige deploy real em staging (`http://178.105.54.0:3006`), fora do alcance de uma sessão local — depende de merge em `develop` + autodeploy + acesso ao VPS. Código pronto e testado localmente (T043/T044); esta task fica para execução manual pós-merge, conforme o fluxo canônico feature→develop→staging→main do AGENTS.md.
