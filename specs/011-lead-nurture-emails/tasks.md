...
- [X] T001 Criar o diretório `src/leadNurture/` (vazio) e confirmar que `src/email/mailer.js`
...
- [X] T002 [P] Adicionar `nurture_email_sent` e `nurture_unsubscribed` à allowlist
      `ANALYTICS_EVENTS` em `src/analytics.js` (contracts/analytics-events.md). Não tocar em
      `PUBLIC_ANALYTICS_EVENTS`.
- [X] T003 [P] Criar `src/leadNurture/unsubscribeToken.js`: `signUnsubscribeToken(userId, secret)` e
...
- [X] T004 [P] Criar `src/email/nurtureEmails.js`: constante `NURTURE_STEPS` (steps 0/2/5/7,
...
- [X] T005 [P] Criar `src/leadNurture/policy.js`: `isRealEmail(email)` (rejeita `@sistema.com` e
...
- [X] T006 [P] Escrever `test/lead-nurture-policy.test.js`: `isRealEmail` (real, `@sistema.com`,
      ...
      (depende de T005).
- [X] T007 [P] Escrever `test/nurture-emails.test.js`: cada um dos 4 passos gera `subject/text/html`
      pt-BR com marca BOTinho e `unsubscribeUrl` presente em `text` **e** `html` (depende de T004).
- [X] T008 [P] Escrever a parte de round-trip do token em `test/lead-nurture-unsubscribe.test.js`:
...
- [X] T009 [US1] Implementar `runNurtureSweep({db, sendMail, now, logger, secret, baseUrl})` em
...
- [X] T010 [US1] Adicionar `startLeadNurtureSweep()` em `src/api/server.js`: roda `runNurtureSweep`
...
- [X] T011 [US1] Em `src/api/routes/auth.js`, logo após o `sendWelcomeEmail(...)` fire-and-forget no
...
- [X] T012 [US1] Escrever `test/lead-nurture-sweep.test.js` (parte 1 — trilha feliz): com `db`/
...
- [X] T013 [US2] Criar `src/api/routes/leadNurture.js` com `GET /api/lead-nurture/unsubscribe`:
...
- [X] T014 [US2] Registrar a rota pública (sem auth) de `src/api/routes/leadNurture.js` no
      `src/api/server.js`, junto ao registro das demais rotas.
- [X] T015 [US2] Em `src/api/routes/auth.js`, condicionar a semeadura do passo 0 (T011) a
...
- [X] T016 [P] [US2] Completar `test/lead-nurture-unsubscribe.test.js` (parte 2 — rota HTTP): token
...
- [X] T017 [US2] Completar `test/lead-nurture-sweep.test.js` (parte 2 — opt-out): um lead com
...
- [X] T018 [US3] Completar `test/lead-nurture-sweep.test.js` (parte 3 — dupla execução): rodar
...
- [X] T019 [US3] Completar `test/lead-nurture-sweep.test.js` (parte 4 — reinício simulado): com
...
- [X] T020 [US3] Completar `test/lead-nurture-sweep.test.js` (parte 5 — passada perdida): com
      `elapsedDays=6` e nada enviado além do passo 0, confirmar que dia 2 e dia 5 saem juntos na
      mesma passada (FR-011).
- [X] T021 [US3] Completar `test/lead-nurture-sweep.test.js` (parte 6 — no-op sem SMTP): com
...
- [X] T022 [US3] Completar `test/lead-nurture-sweep.test.js` (parte 7 — isolamento por item): um
...
- [X] T023 [US3] Executar as verificações de restrições canônicas do quickstart.md §2: `grep
...
- [X] T024 [P] Executar as verificações "zero DDL" do quickstart.md §3: `git status --porcelain
      ...
      confirma FR-008.
- [X] T025 [P] Documentar, como comentário perto de `startLeadNurtureSweep()` em
...
- [X] T026 Rodar a suíte completa: `node --test test/nurture-emails.test.js
