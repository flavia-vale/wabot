...
**Contexto**: Review da implementação (2026-07-22) contra spec.md/plan.md e AGENTS.md.
Achados abaixo em ordem de severidade. As tasks NÃO renumeram/reescrevem nada acima.

- [X] T040 [US3] Corrigir o sinal durável `ops_ml_patch_persist_failed` que é **inatingível em produção** (bloqueia FR-011/FR-012/SC-004). Os dois handlers reais de `__onCredentialPatch` — `attachCredentialPatchHandler` em `src/api/routes/linkConversion.js` (~linha 57-66) e o handler em `src/bot-worker.js` `loadConfig` (~linha 527-538) — já envolvem `persistCredentialPatch` num `try/catch` interno que **engole a exceção** e faz só `logger.warn(...)` **sem relançar**. Logo o `try/catch` novo em torno de `await mlCredentials.__onCredentialPatch(...)` em `src/converters/productInfoScraper.js` (`persistMlCookieRotation` ~linha 176 e o ramo OAuth em `fetchMercadoLivreItemInfo` ~linha 400) **nunca** entra no `catch` em produção, e `recordOperationalSignal('ml_patch_persist_failed', ...)` jamais é emitido nas rotas reais. Os testes T025/T026 passam apenas porque injetam um stub de `__onCredentialPatch` que LANÇA — o que não reflete o comportamento de produção. Correção: emitir o `logger.warn` + `recordOperationalSignal('ml_patch_persist_failed', { axis })` **dentro do próprio `catch` de `persistCredentialPatch`** nos dois handlers de produção (linkConversion + bot-worker), reutilizando `recordOperationalSignal` (módulo leaf, seguro para import), mantendo o contrato best-effort (não relançar). Alternativamente, fazer o `__onCredentialPatch` de produção relançar a exceção para que o `catch` do scraper a capture — mas nesse caso auditar todos os consumidores para não quebrar outros fluxos. Preservar `{ axis: 'cookie' | 'oauth' }` sem vazar segredo na metadata.
- [X] T041 [US3] Estender a cobertura para travar a regressão de T040 com um teste que exercite o handler **real** de `__onCredentialPatch` (não o stub que lança): forçar `persistCredentialPatch` a falhar (ex.: mock de `db.credential.update` lançando `SQLITE_BUSY`) e confirmar que `recordOperationalSignal('ml_patch_persist_failed', ...)` é chamado a partir do handler de `linkConversion.js`/`bot-worker.js`, provando que o sinal durável é atingível pelo caminho de produção — não só pelo stub sintético de T025/T026.
- [X] T042 [US2] Mitigar a degeneração da chave de lock para credenciais **OAuth-only** no double-check (FR-007/FR-009, robustez). `getMlUserToken` passou a envolver o refresh em `withMercadoLivreCredentialLock(mlCredentials, ...)`, mas `mercadoLivreCredentialLockKey` deriva a chave de `creds.id`/`ssid`/`cookie` (`src/converters/mercadolivreCredentialLock.js`). Uma credencial que só tem OAuth (sem `ssid`/`id`/`cookie` — cenário previsto em spec "Snapshot sem cookie (só OAuth)") gera chave constante = `sha256('|')`, serializando o refresh OAuth de **usuários diferentes** sob o MESMO lock (contenção cross-user, risco de `ML_AFFILIATE_LOCK_TIMEOUT` derrubando o token → scrape degradado). Correção sugerida: incluir `userId` na composição da chave de lock quando disponível (ou usar `userId` como componente primário para o eixo OAuth), sem alterar a chave do eixo afiliado já em produção; cobrir com teste db-free que confirme chaves distintas por `userId`.
