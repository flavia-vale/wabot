import { CAPACITY_POLICY_VERSION, DEFAULT_SESSION_COST_FLOOR_MB } from './policy.js'
...
export function calculateCapacityScenario(input, baseline = {}) {
  const valid = validateCapacityScenarioInput(input); const currentSessions = Math.max(0, Math.trunc(Number(baseline.currentSessions) || 0)); const safeLimit = Math.max(0, Math.trunc(Number(baseline.safeLimit) || 0)); const sessionCostMb = Math.max(150, Math.ceil(Number(baseline.sessionCostMb) || DEFAULT_SESSION_COST_FLOOR_MB))
