...
async function createMercadoPagoPreference({ userId, plan }) {
  ...
  const dashboardReturnUrl = `${dashboardUrl}/dashboard/planos`
  if (IS_PRODUCTION && !isHttpsUrl(dashboardReturnUrl)) {
    const err = new Error('DASHBOARD_URL deve usar HTTPS para retorno do checkout')
    err.code = 'PAYMENT_PROVIDER_MISCONFIGURED'
    throw err
  }
  ...
  const preference = {
    ...
    back_urls: {
      success: `${dashboardReturnUrl}?checkout=approved`,
      failure: `${dashboardReturnUrl}?checkout=rejected`,
      pending: `${dashboardReturnUrl}?checkout=pending`,
    },
