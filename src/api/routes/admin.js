...
export async function adminRoutes(app) {
  ...
  app.get('/telegram/overview', async (req, reply) => {
    if (!(await requireAdmin(req, reply, 'tech:read'))) return
    ...
    const [total, count24h, count7d, statusGroups, platformGroups, withImageSuccess, lastRow, distinctChats] = await Promise.all([
      db.telegramOfferLog.count().catch(() => 0),
      db.telegramOfferLog.count({ where: { createdAt: { gte: since24h } } }).catch(() => 0),
      db.telegramOfferLog.count({ where: { createdAt: { gte: since7d } } }).catch(() => 0),
      db.telegramOfferLog.groupBy({ by: ['status'], _count: { _all: true } }).catch(() => []),
      db.telegramOfferLog.groupBy({ by: ['platform'], where: { status: 'success' }, _count: { _all: true } }).catch(() => []),
      db.telegramOfferLog.count({ where: { status: 'success', withImage: true } }).catch(() => 0),
      db.telegramOfferLog.findFirst({ orderBy: { createdAt: 'desc' } }).catch(() => null),
      db.telegramOfferLog.findMany({ distinct: ['chatId'], select: { chatId: true } }).catch(() => []),
    ])

    const statusCounts = { success: 0, product_not_found: 0, invalid_input: 0, error: 0 }
    for (const group of statusGroups) {
      const key = String(group.status)
      statusCounts[key] = (statusCounts[key] || 0) + Number(group._count?._all ?? 0)
    }

    const successCount = statusCounts.success
    // Taxa de sucesso considera só tentativas reais de oferta (ignora
    // mensagens sem link válido / com múltiplos links).
    const offerAttempts = successCount + statusCounts.product_not_found + statusCounts.error
    const successRate = offerAttempts > 0 ? Math.round((successCount / offerAttempts) * 1000) / 10 : 0
    const withImageRate = successCount > 0 ? Math.round((withImageSuccess / successCount) * 1000) / 10 : 0

    const platforms = platformGroups
      .map(group => ({ platform: group.platform || 'desconhecida', count: Number(group._count?._all ?? 0) }))
      .sort((a, b) => b.count - a.count)

    const tokenConfigured = Boolean(String(process.env.TELEGRAM_OFFER_BOT_TOKEN || '').trim())
    const allowedChatIds = String(process.env.TELEGRAM_OFFER_BOT_ALLOWED_CHAT_IDS || '')
      .split(',').map(value => value.trim()).filter(Boolean)

    await writeAdminAuditLog(req, { action: 'admin.telegram.overview.read', resource: 'telegramOffer' })

    return {
      config: {
        tokenConfigured,
        allowedChatCount: allowedChatIds.length,
        openToAllChats: allowedChatIds.length === 0,
      },
      totals: { total, count24h, count7d, distinctChats: distinctChats.length },
      statusCounts,
      successRate,
      withImageRate,
      platforms,
      lastEventAt: lastRow?.createdAt ?? null,
    }
  })

  app.get('/telegram/requests', async (req, reply) => {
    if (!(await requireAdmin(req, reply, 'tech:read'))) return
    ...
    const requests = await db.telegramOfferLog.findMany({
      where,
      orderBy: { createdAt: 'desc' },
      take: limit,
    }).catch(() => [])

    await writeAdminAuditLog(req, { action: 'admin.telegram.requests.list', resource: 'telegramOffer' })
    return { requests }
  })
