...
export async function convert(url, creds) {
  ...
  try {
    ...
    if (hasCookies) {
      ...
      // renovar os cookies pra voltar a gerar amzn.to.
      logger.warn({ url, longUrl, transient }, 'Amazon: API não retornou shortUrl — fallback para ?tag= (renovar cookies sitestripe)')
