...
```bash
mkdir -p /home/deploy/BOTinho-shared/auth_info /home/deploy/BOTinho-shared/logs
chmod 700 /home/deploy/BOTinho-shared/auth_info /home/deploy/BOTinho-shared/logs
```
...
cd /home/deploy/BOTinho/dashboard
npm ci
npm run build
cd /home/deploy/BOTinho
...
```nginx
upstream BOTinho_api {
  server 127.0.0.1:3001 max_fails=3 fail_timeout=10s;
  keepalive 32;
}

upstream BOTinho_dashboard {
...
server {
  ...
  location /health {
    proxy_pass http://BOTinho_api/health;
  }

  location / {
    proxy_pass http://BOTinho_dashboard;
    proxy_connect_timeout 2s;
    proxy_send_timeout 30s;
    proxy_read_timeout 30s;
  }
...
- Production: `/home/deploy/BOTinho`, branch `main`, `API_PORT=3001`, dashboard `PORT=3000`, `AUTH_INFO_DIR=/home/deploy/BOTinho-shared/auth_info`.
- Staging: `/home/deploy/BOTinho-staging`, branch de teste, `API_PORT=3101`, dashboard `PORT=3100`, `AUTH_INFO_DIR=/home/deploy/BOTinho-staging-shared/auth_info`.
...
- Se `AUTH_INFO_DIR` for alterado para um diretório novo sem copiar as sessões antigas, os usuários precisarão ler QR novamente. Migre com `cp -a /home/deploy/BOTinho/auth_info/. /home/deploy/BOTinho-shared/auth_info/` antes do reload.
