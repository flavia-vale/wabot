...
| D4 | busca `site:cuponito.com.br` (ferramenta de busca da sessão) | só apareceram URLs do site **antigo** (WordPress: `/store/<loja>/`, `/stores-2/`, título "Os melhores cupons? O Cuponito acha!"), que hoje devolvem 404 | a autoridade que o domínio já tinha está apontando para 404. Se as URLs novas (`/desconto/*`, `/blog/*`) estão indexadas **não dá para medir de fora** — conferir no Bing Webmaster Tools e no Search Console |
