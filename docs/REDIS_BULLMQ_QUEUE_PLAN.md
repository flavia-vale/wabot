...
A fila atual do Wabot é em memória por processo `bot-worker`. Ela reduz latência do listener, mas não sobrevive a restart. Para muitos clientes simultâneos ou necessidade de garantia de entrega, migrar para Redis + BullMQ.
...
- Uma fila BullMQ por ambiente: `wabot-send-production`.
