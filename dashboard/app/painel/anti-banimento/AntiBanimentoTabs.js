...
export default function AntiBanimentoTabs({ parte, destino }) {
  ...
  return (
    <div className="pnl-grid" style={{ maxWidth: 1080, margin: '0 auto' }}>
      <nav className="pnl-seg" aria-label="Partes do Anti-banimento" style={{ justifySelf: 'start' }}>
        {PARTS.map((p) => (
          <Link
            key={p.value}
            href={hrefFor(p.value, destino)}
            className={`rounded-full px-4 py-2 text-sm font-semibold no-underline transition ${parte === p.value ? 'bg-gray-900 text-white' : 'text-gray-500 hover:bg-gray-100 hover:text-gray-900'}`}
