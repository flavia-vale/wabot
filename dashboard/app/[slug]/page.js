...
export function generateStaticParams() { return Object.keys(LP_CONFIG).map((slug) => ({ slug })) }

export function generateMetadata({ params }) {
  const cfg = LP_CONFIG[params.slug]
  if (!cfg) return {}
  return { title: cfg.title, description: cfg.description, alternates: { canonical: `/${params.slug}` } }
}

export default function ProgrammaticLpPage({ params }) {
  const cfg = LP_CONFIG[params.slug]
  if (!cfg) notFound()
