...
function getRoutineExample(slug, cfg, lpType) {
  ...
  if (lpType === 'niche') {
    ...
    return {
      ...
      body: `Uma operação de ${niche.toLowerCase()} pode priorizar ofertas por margem, estoque e urgência real. O Espelha Grupos entra depois da curadoria: organiza origem, destino, filtros e cadência para que a mensagem certa chegue aos grupos e canais adequados com revisão humana.`,
...
export function LpTemplate({ slug }) {
  ...
  const howToJsonLd = { '@context': 'https://schema.org', '@type': 'HowTo', name: `Como configurar ${cfg.title.replace('', '')}`, step: cfg.howTo.map((text, index) => ({ '@type': 'HowToStep', name: `Passo ${index + 1}`, text })) }
  const productJsonLd = {
  ...
  const evidenceCard = {
    title: `Sinal de evidência: ${cfg.title.replace('', '')}`,
      ...
      <div aria-labelledby={`${slug}-faq-especifica`} role="region">
        <div className="wrap" style={{ marginTop: 28 }}>
          <div style={{ background: 'var(--surface)', border: '1px solid var(--line)', borderRadius: 24, padding: 28 }}>
            ...
            >
              {`Perguntas específicas sobre ${cfg.title.replace('', '')}`}
            </h2>
