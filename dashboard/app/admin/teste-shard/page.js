...
export default function ShardPocPage() {
  ...
  const totals = useMemo(() => (data?.members || []).reduce((out, member) => {
    out.rss += Number(member.runtime?.rssBytes || 0); out.heap += Number(member.runtime?.heapUsedBytes || 0); out.external += Number(member.runtime?.externalBytes || 0); out.failures += Number(member.failures24h || 0)
    return out
  }, { rss: 0, heap: 0, external: 0, failures: 0 }), [data])
  ...
  return <main className="min-h-screen bg-slate-950 px-4 py-8 text-slate-100 sm:px-6"><div className="mx-auto max-w-7xl space-y-5">
    <header className="flex flex-wrap items-start justify-between gap-4"><div><p className="text-xs font-black uppercase tracking-[.25em] text-violet-300">ADMIN · TESTE CONTROLADO · PRODUÇÃO</p><h1 className="mt-1 text-3xl font-black">Shard WhatsApp · 4 contas</h1><p className="mt-2 max-w-3xl text-sm text-slate-300">Observação ao vivo das quatro contas candidatas. Esta tela não move nem reinicia sessões: o cutover permanece bloqueado até o runtime multi-sessão e o rollback automático estarem validados.</p></div><Link href="/admin" className="flex min-h-11 items-center rounded-xl border border-slate-700 bg-slate-900 px-4 text-sm font-bold">Voltar ao ADMIN</Link></header>
