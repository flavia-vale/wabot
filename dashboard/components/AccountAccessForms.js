...
/* E-mail de acesso, senha e (premium) Instagram. Extraído de
 * /painel/configuracoes para ser o MESMO bloco em Minha conta — duas cópias
 * do formulário de senha passariam a discordar em silêncio. */
export default function AccountAccessForms({ embedded = false }) {
  // Dentro de Minha conta os blocos entram na grade da própria página.
  const wrapStyle = embedded ? { display: 'contents' } : { maxWidth: 760, margin: '0 auto' }
  ...
  if (loadError) {
    return (
      <div className="pnl-grid" style={wrapStyle}>
  ...
  if (!account) {
    return <div className="pnl-grid" style={wrapStyle}><div className="pnl-skel" style={{ height: 360 }} /></div>
  }
