...
const STORY_STATUS = { queued: 'Na fila', processing: 'Publicando', container_processing: 'Preparando', published: 'Publicado', failed: 'Falhou', retry_scheduled: 'Nova tentativa agendada', reconciliation_required: 'Conferência necessária', cancelled: 'Cancelado' }
    ...
    <section className="pnl-card" aria-labelledby="instagram-title">
      <div style={{ display: 'flex', gap: 16, justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap' }}>
        <div>
          ...
          <p className="pnl-card-note" style={{ marginTop: 6, maxWidth: 520 }}>Conecte uma conta profissional para publicar ofertas no formato vertical. Disponível somente no futuro plano acima do Pro.</p>
        </div>
      ...
      {health && <div className={`pnl-note-box ${health.requiresAttention ? 'is-error' : 'is-success'}`} role="status" style={{ marginTop: 16 }}>
        ...
        <p style={{ marginTop: 4 }}>Na fila: {health.publications.queued} · Publicando: {health.publications.processing} · Publicados: {health.publications.published} · Falhas no espelhamento: {health.mirroring.failed}</p>
        {health.publications.reconciliation > 0 && <p style={{ marginTop: 4 }}>Há {health.publications.reconciliation} publicação(ões) aguardando conferência para evitar duplicidade.</p>}
      </div>}

      <div style={{ display: 'grid', gap: 10, marginTop: 18 }}>
        {connections.length === 0 && <div className="pnl-note-box">Nenhuma conta conectada. A conta precisa ser Business ou Creator.</div>}
      ...
      <div style={{ marginTop: 24 }}>
        ...
        <div style={{ overflowX: 'auto', marginTop: 10 }}>
          <table style={{ width: '100%', minWidth: 560, borderCollapse: 'collapse' }}>
            ...
            <tbody>{stories.slice(0, 20).map(story => <tr key={story.id} style={{ borderTop: '1px solid var(--line)' }}><td style={{ padding: '10px 0' }}>{story.destination?.name || 'Instagram'}</td><td>{SOURCE[story.sourceType] || 'Oferta'}</td><td>{STORY_STATUS[story.status] || 'Em análise'}</td><td>{date(story.publishedAt || story.scheduledFor || story.createdAt)}</td><td style={{ display: 'flex', gap: 6 }}>{['failed', 'retry_scheduled'].includes(story.status) && <button className="pnl-btn" onClick={() => act('retry', story.id)} disabled={Boolean(busy)}>Tentar novamente</button>}{story.status === 'queued' && story.scheduledFor && <button className="pnl-btn" onClick={() => act('cancel', story.id)} disabled={Boolean(busy)}>Cancelar</button>}</td></tr>)}</tbody>
          </table>
        </div>
      </div>
    </section>
