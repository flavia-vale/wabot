...
export function explainReviewDiscovery(result = {}) {
  ...
  if (Number(result.discovered) > 0) return { tone: 'success', text: result.replaced ? `${result.discovered} nova(s) opção(ões) encontrada(s). As ofertas que você já aprovou foram mantidas.` : `${result.discovered} oferta(s) adicionada(s) à fila.` }
